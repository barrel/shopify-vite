    ...
    configureServer ({ config, middlewares, httpServer }) {
      httpServer?.once('listening', () => {
        ...
        if (isAddressInfo(address)) {
          ...
          const viteTagSnippetContent = viteTagDisclaimer + themeCheckDisable + viteTagEntryPath(config.resolve.alias, options.entrypointsDir, viteTagSnippetName) + viteTagSnippetDev(viteDevServerUrl, options.entrypointsDir, reactPlugin) + themeCheckEnable
...
// Disable Shopify theme-check linter at top of file
const themeCheckDisable = '{% comment %}theme-check-disable AssetUrlFilters,RemoteAsset{% endcomment %}\n'

// Re-enbale Shopify theme-check linter at end of file
const themeCheckEnable = '{% comment %}theme-check-enable AssetUrlFilters,RemoteAsset{% endcomment %}\n'
